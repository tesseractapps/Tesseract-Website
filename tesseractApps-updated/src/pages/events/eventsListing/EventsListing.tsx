...
const EVENTS: EventItem[] = [
  {
    ...
    subtitle: "Meet the TesseractApps team, see a live platform demo, and enter our prize draw to win 12 months free.",
    ...
    <div id="ev-page">
      ...
      <section id="ev-content">
        <div id="ev-outer">
                ...
                {upcomingEvents.map(ev => (
                  <Link key={ev.slug} to={ev.slug} className="ev-card">
                    <div className="ev-card-header">
                      <span className="ev-card-tag ev-card-tag--upcoming">{ev.tag}</span>
                    ...
                    <div className="ev-card-details">
                      ...
                      <div className="ev-card-detail">
                        <MapPin size={14} />
                        <span>{ev.venue}{ev.booth ? ` · ${ev.booth}` : ""}</span>
                      </div>
                    </div>
                    <div className="ev-card-footer">
                      <span className="ev-card-cta">Register &amp; Enter to Win →</span>
                    </div>
                  </Link>
                ))}
              </div>
          ...
          {pastEvents.length > 0 && (
            <>
              <div className="ev-group-label ev-group-label--past">Past Events</div>
              <div className="ev-grid ev-grid--past">
                {pastEvents.map(ev => (
                  <div key={ev.slug} className="ev-card ev-card--past">
                    <div className="ev-card-header">
                      <span className="ev-card-tag">{ev.tag}</span>
                      <span className="ev-card-status ev-card-status--past">Past</span>
                    </div>
                    <h2 className="ev-card-title">{ev.title}</h2>
                    <p className="ev-card-subtitle">{ev.subtitle}</p>
                    <div className="ev-card-details">
          ...
          {/* ── Empty state ── */}
          {EVENTS.length === 0 && (
            <p className="ev-empty">No events scheduled yet. Check back soon.</p>
          )}
