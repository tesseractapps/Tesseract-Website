...
const pWithLink = (before, linkText, href, after) => block('normal', [
  span(before),
  { _type: 'span', _key: key(), text: linkText, marks: [key()] },
  span(after),
], undefined, undefined)

// For simplicity with external links we keep them as plain text with the URL inline
// (rich links require markDefs wiring — done below for internal links)
const pLinked = (text, links) => {
  // links: [{ mark, href, text }]
  const markDefs = links.map(l => ({ _type: 'link', _key: l.mark, href: l.href }))
  const children = []
  let remaining = text
  for (const l of links) {
    const idx = remaining.indexOf(l.text)
    if (idx > 0) children.push(span(remaining.slice(0, idx)))
    children.push({ _type: 'span', _key: key(), text: l.text, marks: [l.mark] })
    remaining = remaining.slice(idx + l.text.length)
  }
  if (remaining) children.push(span(remaining))
...
const doc = {
  ...
  featured: false,
  cta: {
    heading: 'Download our free SIL Compliance Readiness Checklist',
    body: 'A practical tool designed to help you identify where you stand and what needs attention before July 2026. Or book a free demo to see how our software keeps you audit-ready as standards evolve.',
  ...
  body: [
    ...
    pLinked(
      'The updated standards require a documented business continuity plan (BCP) that includes participant communication strategies and staff training for critical incidents. Auditors will expect to see evidence that your BCP has been tested, not just written. Read our full guide on what NDIS auditors now expect from your BCP.',
      [{ mark: mk, href: '/blog/ndis-business-continuity-plan-audit-2026', text: 'what NDIS auditors now expect from your BCP' }]
    ),
    ...
    pLinked(
      'Download the Commission\'s Insights Report and familiarise yourself with the direction of the new SIL standard.',
      [{ mark: mk2, href: 'https://www.ndiscommission.gov.au', text: "Commission's Insights Report" }]
    ),
