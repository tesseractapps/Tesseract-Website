...
import { BROCHURES_QUERY } from '../sanity/lib/queries'
...
const cache = new Map<string, BrochureItem[]>()
const CACHE_KEY = 'brochures:all'

type UseSanityBrochuresResult = {
  data: BrochureItem[]
  loading: boolean
  error: string | null
}

export function useSanityBrochures(): UseSanityBrochuresResult {
  const [data, setData] = useState<BrochureItem[]>(() => cache.get(CACHE_KEY) ?? [])
  ...
  useEffect(() => {
    ...
    client
      .fetch<BrochureItem[]>(BROCHURES_QUERY)
      .then((result) => {
        if (!cancelled) {
          const items = result ?? []
